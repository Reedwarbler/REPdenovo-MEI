...
def construct_ref(sf_ref, m_pos, mhap, sf_new_ref, sf_new_pos):
    l_new_seqs=[]
    m_new_pos={}
    for record in SeqIO.parse(sf_ref, "fasta"):
        ...
        for start,end in m_pos[sid]:
            ...
            #accumulation=accumulation+(end-start+1)
...
def check_one_pos(sf_bam, chrm, istart):
    sf_out="tmp2/{0}_{1}.sam".format(chrm, istart)
...
def check_individual_has_rep(m_chrom_pos, sf_bam, sf_rslt):
    ...
    for (chrm, istart, iend) in l_sites:
        sf_out="tmp2/{0}_{1}.sam".format(chrm, istart)
        cmd="samtools view {0} {1}:{2}-{3} > {4}".format(sf_bam, chrm, istart-1000, istart+1000, sf_out)
        sf_out1="tmp2/{0}_{1}.sam".format(chrm, iend)
...
if __name__ == "__main__":
    ...
    #sf_hap1=sys.argv[3]
    #sf_hap1_pos=sys.argv[4]
    #sf_hap2=sys.argv[5]
    #sf_hap2_pos=sys.argv[6]
    len_cutoff=50
    #len_cutoff=400
    percent=25
    #
    ...
    ##mhap1, mhap2=gnrt_genotype(m_chrom_pos, percent, sf_gntp)
    ###mhap1,mhap2=load_genotype_from_file(sf_gntp)

    #construct_ref(sf_ref, m_chrom_pos, mhap1, sf_hap1, sf_hap1_pos)
