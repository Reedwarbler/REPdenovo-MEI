...
class REPLocator:
    ...
    def cnt_discordant_support(self, potential_discordant_pe_pos, clip_pos):
        cnt=0
        for i in range(clip_pos-self.dist2, clip_pos+self.dist2+1):
    ...
    def locate_rep_sites_clip(self, sf_high_freq_kmers, sf_simple_rep, anchor_mapq, pe_cutoff,
        ...
        for sam_record in sys.stdin:
            ...
            #check clipped reads
            bqualified_clip,cnt_m, clip_flag, left_clip_len, right_clip_len=self.is_qualified_clipped(cigar, min_clip_lenth)
            ...
            if left_clip_len>0:
                ...
                n_left_hit_simple=self.cnt_hit_kmer_lib(m_sim_rep_kmer, left_clip_seq,k)
            ...
            if right_clip_len>0:
                rd_len=len(seq)
                right_clip_seq=self.preprocess_seq(m_alpha, seq[rd_len-right_clip_len:])
                ...
                n_right_hit_simple=self.cnt_hit_kmer_lib(m_sim_rep_kmer, right_clip_seq,k)
        ...
        merged_left=self.cluster_sites(candidate_sites_left, n_extend)
        merged_right=self.cluster_sites(candidate_sites_right, n_extend)

        candidate_sites={}
        candidate_site_simple_rep={}
        for pos in merged_left:
            if candidate_sites.has_key(pos)==False and merged_left[pos]>=clip_cutoff:
                n_discordant=self.cnt_discordant_support(potential_discordant_pe_pos, pos)
                if n_discordant >= pe_cutoff:
                    candidate_sites[pos]=merged_left[pos]
                    if candidate_simple_left.has_key(pos)==False:
                        print pos, merged_left[pos], n_discordant, "left"
                    else:
            ...
            if candidate_sites.has_key(pos)==False and merged_right[pos]>=clip_cutoff:
                ...
                if n_discordant >= pe_cutoff:
                    ...
                    if candidate_simple_right.has_key(pos)==False:
                        print pos, merged_right[pos], n_discordant, "right"
                    else:
...
if __name__ == "__main__":
    ...
    min_clip_lenth=21
    k=21
    hit_ratio=float(sys.argv[3]) #0.5
    n_extend=20
    clip_cutoff=int(sys.argv[4]) #5
    anchor_mapq=30
    pe_cutoff=int(sys.argv[5]) #3

    insert_size=300
    derivation=50

    rl=REPLocator(insert_size, derivation)
    rl.locate_rep_sites_clip(sf_high_freq_kmers, sf_simple_rep, anchor_mapq, pe_cutoff,
                          min_clip_lenth, k, hit_ratio, n_extend, clip_cutoff)